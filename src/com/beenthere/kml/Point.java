...
public class Point extends Geometry {
	...
	public Node handleTag(String tag, String text) {
		if (tag.equals("coordinates") && text != null) {
	...
	public void setCoordinates(String value) {
		...
		for (int i = 0; i < coords.length; i++) {
			try {
				...
				System.err.println("Error parsing double (" + coords[i]
						+ ") in Point.updateNumericalCoordinates " + nfe);
			}
		}
	...
	/**
	 * @param numericalCoordinates
	 *            the numericalCoordinates to set
	 */
	...
	public void addNumericalCoordinate(double numericalCoordinate) {
		...
		System.arraycopy(this.coordinates, 0, newCoords, 0,
	...
	public String toKML(boolean suppressEnclosingTags) {
		...
		if (this.altitudeMode != null) {
			kml += "<altitudeMode>"
					+ SpecialCaseFormatter.toKMLString(this.altitudeMode)
					+ "</altitudeMode>\n";
		}
